...
public class GridModel<State> {
    ...
    private void init() {
        ...
        for (int col = 0; col < numberOfColumns.get(); col++) {
            for (int row = 0; row < numberOfRows.get(); row++) {
                Cell<State> existingCell = getCell(col, row);

                if (existingCell == null) {
                    Cell<State> cell = new Cell<>(col, row);
    ...
    public Cell<State> getCell(int column, int row) {
        return cells.stream().filter(cell -> (cell.getColumn() == column && cell.getRow() == row)).findFirst().orElse(null);
    }
